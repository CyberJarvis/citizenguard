...
const ForecastConeLayer = dynamic(
  () => import('@/components/map/ForecastConeLayer').then((mod) => mod.ForecastConeLayer),
  { ssr: false }
);

const VulnerabilityLayer = dynamic(
  () => import('@/components/map/VulnerabilityLayer'),
  { ssr: false }
);

const SOSMarkerLayer = dynamic(
  () => import('@/components/map/SOSMarkerLayer'),
  { ssr: false }
);

const SOSButton = dynamic(
  () => import('@/components/sos/SOSButton'),
  { ssr: false }
);
...
const OfflineMapSettings = dynamic(
  () => import('@/components/map/OfflineMapSettings'),
  { ssr: false }
);

// Timeline filter component
import { TimelineFilter } from '@/components/map/TimelineFilter';
...
function OceanMapContent() {
  ...
  // Timeline state
  const [timelineRange, setTimelineRange] = useState(null); // null = default 24h, '6h', '24h', '48h_future'
  const [isTimelineLoading, setIsTimelineLoading] = useState(false);

  // Vulnerability layers state
  const [showPorts, setShowPorts] = useState(false);
  const [showFishingVillages, setShowFishingVillages] = useState(false);
  const [showMarineAreas, setShowMarineAreas] = useState(false);
  const [showPopulationDensity, setShowPopulationDensity] = useState(false);
  const [showCoastGuardStations, setShowCoastGuardStations] = useState(false);

  // SOS layer state
  const [showSOSMarkers, setShowSOSMarkers] = useState(true);
  ...
  const {
    ...
    hoursFilter: 24,
    autoRefresh: true,
    timelineRange: timelineRange, // Pass timeline range to hook
  });

  // Handle timeline range change
  const handleTimelineChange = useCallback((range) => {
    setIsTimelineLoading(true);
    setTimelineRange(range);
    // Loading state will be cleared when data updates
    setTimeout(() => setIsTimelineLoading(false), 1500);
  }, []);
  ...
  return (
    <div className="map-page-container">
      {/* ============== OFFLINE BANNER ============== */}
      <OfflineBanner className="fixed top-0 left-0 right-0 z-[1100]" />
      ...
      >
        ...
        {/* Forecast Cone Layer - shown when timeline is set to future */}
        ...
        {/* Vulnerability/Infrastructure Layers */}
        <VulnerabilityLayer
          showPorts={showPorts}
          showFishingVillages={showFishingVillages}
          showMarineAreas={showMarineAreas}
          showPopulationDensity={showPopulationDensity}
          showCoastGuardStations={showCoastGuardStations}
        />
        {/* SOS Emergency Markers */}
        <SOSMarkerLayer
          visible={showSOSMarkers}
      ...
      <div className={`map-left-nav ${leftNavOpen ? 'open' : 'closed'}`}>
        ...
        <nav className="left-nav-content">
          <a href="/dashboard" className="left-nav-item">
            <Home className="w-5 h-5" />
            <span>Dashboard</span>
          </a>
          <a href="/report-hazard" className="left-nav-item highlight">
            <AlertTriangle className="w-5 h-5" />
            <span>Report Hazard</span>
          </a>
          <a href="/my-reports" className="left-nav-item">
            <FileText className="w-5 h-5" />
            <span>My Reports</span>
          </a>
          <a href="/map" className="left-nav-item active">
            <Map className="w-5 h-5" />
            <span>Map View</span>
          </a>
          <a href="/community" className="left-nav-item">
            <MessageCircle className="w-5 h-5" />
            <span>Community</span>
          </a>
          <a href="/leaderboard" className="left-nav-item">
            <Trophy className="w-5 h-5" />
            <span>Leaderboard</span>
          </a>
          <a href="/safety" className="left-nav-item">
          ...
          {activeTab === 'layers' && (
            <div className="tab-content">
              ...
              <div className="layer-section">
                <TimelineFilter
                  selectedRange={timelineRange || '24h'}
                  onRangeChange={handleTimelineChange}
                  isLoading={isTimelineLoading || isRefreshing}
                  disabled={isLoading}
                />
              </div>
              ...
              <div className="layer-section">
                ...
                <div className="toggle-list">
                  ...
                  <label className="toggle-item">
                    ...
                    <input
                      type="checkbox"
                      checked={showSOSMarkers}
                      onChange={() => setShowSOSMarkers(!showSOSMarkers)}
                      className="toggle-switch"
                    />
                  </label>
                </div>
              ...
              <div className="layer-section">
                <p className="section-title">Infrastructure Layers</p>
                <div className="toggle-list">
                  <label className="toggle-item">
                    <div className="toggle-info">
                  ...
                  <label className="toggle-item">
                    ...
                    <input
                      type="checkbox"
                      checked={showFishingVillages}
                      onChange={() => setShowFishingVillages(!showFishingVillages)}
                      className="toggle-switch"
                    />
                  </label>
                  <label className="toggle-item">
                    ...
                    <input
                      type="checkbox"
                      checked={showMarineAreas}
                      onChange={() => setShowMarineAreas(!showMarineAreas)}
                      className="toggle-switch"
                    />
                  </label>
                  <label className="toggle-item">
                    ...
                    <input
                      ...
                      onChange={() => setShowPopulationDensity(!showPopulationDensity)}
                      className="toggle-switch"
                    />
                  </label>
                  <label className="toggle-item">
                    <div className="toggle-info">
                      ...
                      <span>Coast Guard Stations</span>
                    </div>
                    <input
                      type="checkbox"
                      checked={showCoastGuardStations}
                      onChange={() => setShowCoastGuardStations(!showCoastGuardStations)}
                      className="toggle-switch"
                    />
                  </label>
                </div>
